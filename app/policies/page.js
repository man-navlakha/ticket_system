'use client';

import { useState } from 'react';
import Link from 'next/link';
import { motion, AnimatePresence } from 'framer-motion';

const defaultPolicies = [
    {
        title: "Data Privacy & Confidentiality",
        rules: "We take your data seriously. All data managed within our enterprise support system is strictly confidential, encrypted, and governed by applicable data protection laws. Employee information is only processed on an as-needed basis for IT resolutions."
    },
    {
        title: "Communication Policies",
        rules: "Internal communication across tickets, comments, and proposals should always maintain a professional tone. Harassment, abusive language, or inappropriate content will lead to immediate suspension from the platform."
    },
    {
        title: "Device Management & Fair Use",
        rules: "Devices are provided to aid your workflow and enhance productivity. Installing unauthorized software, pirated applications, or violating the network's Fair Use policy on these devices is strictly prohibited."
    },
    {
        title: "SLA Adherence",
        rules: "Our support agents pledge to address your queries within predetermined Service Level Agreement (SLA) timelines. Priority issues must be justified, and circumventing standard queue procedures is not permitted."
    },
    {
        title: "Access Permissions",
        rules: "Role-based access controls dictate what data you can view and modify. Attempting to bypass these permissions or escalate privileges through technical exploits is considered a severe enterprise infraction."
    }
];

export default function PoliciesPage() {
    const [openRule, setOpenRule] = useState(null);

    return (
        <div className="min-h-screen bg-background text-foreground font-sans p-8 selection:bg-primary/20 transition-colors duration-300">
            <div className="max-w-4xl mx-auto space-y-12 py-10">
                {/* Header */}
                <div className="text-center space-y-4">
                    <Link href="/" className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors mb-4 border border-border px-4 py-2 rounded-full bg-muted/30">
                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" />
                        </svg>
                        Back to Home
                    </Link>
                    <h1 className="text-4xl md:text-5xl font-semibold tracking-tight text-foreground">Policies</h1>
                    <p className="text-lg text-muted-foreground max-w-2xl mx-auto leading-relaxed">
                        Read our overarching system policies outlining privacy, acceptable use, and standard operational guidelines.
                    </p>
                </div>

                {/* Policies Accordion */}
                <div className="space-y-4">
                    {defaultPolicies.map((rule, i) => (
                        <div key={i} className="border border-border rounded-xl bg-card overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                            <button
                                onClick={() => setOpenRule(openRule === i ? null : i)}
                                className="w-full flex items-center justify-between p-6 text-left hover:bg-muted/30 transition-colors"
                            >
                                <span className="font-medium text-lg text-foreground">{rule.title}</span>
                                <div className={`w-8 h-8 rounded-full border border-border flex items-center justify-center transition-colors ${openRule === i ? 'bg-primary text-primary-foreground border-primary' : 'bg-muted/30 text-muted-foreground'}`}>
                                    <svg className={`w-5 h-5 transition-transform duration-300 ${openRule === i ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M19 9l-7 7-7-7" />
                                    </svg>
                                </div>
                            </button>
                            <AnimatePresence>
                                {openRule === i && (
                                    <motion.div
                                        initial={{ height: 0, opacity: 0 }}
                                        animate={{ height: 'auto', opacity: 1 }}
                                        exit={{ height: 0, opacity: 0 }}
                                        className="overflow-hidden"
                                    >
                                        <div className="p-6 pt-0 text-muted-foreground leading-relaxed border-t border-border mt-2">
                                            {rule.rules}
                                        </div>
                                    </motion.div>
                                )}
                            </AnimatePresence>
                        </div>
                    ))}
                </div>

                {/* Footer Section */}
                <div className="text-center pt-10 border-t border-border mt-12">
                    <p className="text-sm text-muted-foreground">
                        Last updated: {new Date().toLocaleDateString()}
                    </p>
                    <p className="text-sm text-muted-foreground mt-2">
                        To report a potential policy violation, please log into your dashboard and file an appropriate ticket.
                    </p>
                </div>
            </div>
        </div>
    );
}
