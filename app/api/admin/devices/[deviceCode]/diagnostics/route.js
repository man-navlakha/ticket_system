...
export async function GET(request, { params }) {
    ...
    try {
        ...
        const upstream = await fetchLaptopData(
            `/api/admin/devices/${encodeURIComponent(deviceCode)}/diagnostics`,
            { searchParams }
        );

        return proxyJsonResponse(upstream, 'Unable to fetch device diagnostics');
    } catch (error) {
        return laptopDataErrorResponse(error, 'Unable to fetch device diagnostics');
    }
}
