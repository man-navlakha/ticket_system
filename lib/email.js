...
export async function sendInviteEmail(toEmail, inviteLink, role = 'USER') {
    ...
    const footerHtml = `You received this because you were invited to Excellent IT. If you didn't expect this, you can safely ignore it.`;
...
If you didn't expect this invitation, please ignore this email.
    ...
    const mailOptions = {
        from: `"Excellent IT" <${process.env.ZOHO_EMAIL}>`,
        to: toEmail,
...
export async function sendPasswordResetEmail(toEmail, resetLink) {
    ...
    const footerHtml = `You received this because a password reset was requested for your Excellent IT account. If this wasn't you, no action is needed.`;

    const emailHtml = renderWrapper(innerHtml, footerHtml, "Reset your password");

    const emailText = `
Reset your password — Excellent IT
...
export async function sendNewTicketNotification(toEmail, ticketData, baseUrl, replyTo) {
    ...
    const footerHtml = `Automated notification from Excellent IT. Do not reply to this email.`;
...
export async function sendTicketConfirmationToReporter(toEmail, ticketData, itReplyTo) {
    const { id, title, description, priority } = ticketData;
    ...
    const innerHtml = `
        <h2 style="margin: 0 0 12px 0; font-size: 22px; font-weight: 700; color: #09090b; letter-spacing: -0.5px;">We got your report</h2>
        <p style="margin: 0 0 16px 0;">Thanks — your ticket has been logged with the IT team. We will get back to you on this email thread shortly.</p>
        <p style="margin: 0 0 16px 0;"><strong style="color: #09090b;">To continue the conversation, simply reply to this email.</strong> IT will see your reply and respond here.</p>
        ...
        <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="100%" style="margin: 20px 0;">
            <tr>
                <td class="ticket-box-bg" style="background-color: #f4f4f5; border: 1px solid #e4e4e7; border-radius: 8px; padding: 20px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif; font-size: 14px; color: #52525b;">
                    ...
                    <p style="margin: 0;"><strong style="color: #09090b;">What you reported:</strong><br>${String(description).replace(/\n/g, '<br>')}</p>
                </td>
            </tr>
        </table>
    ...
    const footerHtml = `Save this email — replying here keeps the whole conversation in one thread with IT.`;
    const emailHtml = renderWrapper(innerHtml, footerHtml, 'Your ticket has been logged');

    const emailText = `
We got your report — Excellent IT
...
To continue the conversation, simply reply to this email and IT will get back to you here.
    `.trim();
 ...
 * Plain text + minimal HTML — code prominent, no marketing chrome.
 */
export async function sendOtpEmail(toEmail, code, pid) {
    const innerHtml = `
        <h2 style="margin: 0 0 12px 0; font-size: 22px; font-weight: 700; color: #09090b; letter-spacing: -0.5px;">Your verification code</h2>
        <p style="margin: 0 0 12px 0;">Someone scanned the QR for device <strong>${pid || ''}</strong> and asked to see your contact details. If that was you, use this code:</p>
        <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="100%" style="margin: 16px 0;">
            <tr>
                <td class="link-box-bg" style="background-color: #f4f4f5; border: 1px solid #e4e4e7; border-radius: 12px; padding: 18px 24px; text-align: center; font-family: 'SFMono-Regular', Consolas, monospace; font-size: 32px; font-weight: 800; letter-spacing: 12px; color: #09090b;">
                    ${code}
                </td>
            </tr>
        </table>
        <p class="meta-text" style="font-size: 13px; color: #71717a; margin: 0;">This code expires in <strong>10 minutes</strong>. If this wasn't you, ignore this email — your details stay hidden.</p>
    `;
    const footerHtml = `Sent because someone scanned the QR sticker on a device assigned to you.`;
    const emailHtml = renderWrapper(innerHtml, footerHtml, 'Verification code');
